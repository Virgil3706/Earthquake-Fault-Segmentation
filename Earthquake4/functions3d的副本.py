...
def split_Image(bigImage, isMask, top_pad, bottom_pad, left_pad, right_pad, front_pad, back_pad, splitsize, stepsize,
                vertical_splits_number, horizontal_splits_number, deepth_splits_number):
    #     print(bigImage.shape)
    if isMask == True:
        arr = np.pad(bigImage, (((top_pad, bottom_pad), (left_pad, right_pad), (front_pad, back_pad))), "reflect")
        splits = view_as_windows(arr, (splitsize, splitsize), step=stepsize)
        splits = splits.reshape(
            (vertical_splits_number * horizontal_splits_number * deepth_splits_number, splitsize, splitsize))
    else:
        arr = np.pad(bigImage, ((top_pad, bottom_pad), (left_pad, right_pad), (front_pad, back_pad), (0, 0)), "reflect")
        splits = view_as_windows(arr, (splitsize, splitsize, 3), step=stepsize)
        splits = splits.reshape(
            (vertical_splits_number * horizontal_splits_number * deepth_splits_number, splitsize, splitsize, 3))
...
def recover_Image(patches: np.ndarray, imsize: Tuple[int, int, int], left_pad, right_pad, top_pad, bottom_pad,
                  front_pad, back_pad, overlapsize):
    #     patches = np.squeeze(patches)
    assert len(patches.shape) == 5

    i_h, i_w, i_chan = imsize
    image = np.zeros((i_h + top_pad + bottom_pad, i_w + left_pad + right_pad, i_chan + front_pad + back_pad), dtype=patches.dtype)
    divisor = np.zeros((i_h + top_pad + bottom_pad, i_w + left_pad + right_pad, i_chan + front_pad + back_pad), dtype=patches.dtype)
