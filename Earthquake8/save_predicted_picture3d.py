...
def main():
    ...
    model.load_state_dict(torch.load('model_best1.pth',map_location=torch.device('cpu'))['state_dict'])
    cube = np.zeros((64,352,352))
    blocks = []
    print('-'*30)
    ...
    for images in test_set_loader:
        ...
        tosave = torch.stack(outputs).detach().cpu().numpy()
    ...
    for i in range(r, cube.shape[0] - r + 1, step):
        for j in range(r, cube.shape[1] - r + 1, step):
            for k in range(r, cube.shape[2] - r + 1, step):
                cube[i - r:i + r, j - r:j + r, k - r:k + r]+=blocks[x].detach().numpy()
                x=x+1
        # print(res.shape)
    cube_overlap = np.zeros((64,352,352))
    ...
    save_path ='saved/picture/unet3d/new1'
    ...
    for index in range(0,64):
        ...
        plt.imshow(seismic[index+286,0:352,0:352])
        plt.axis('off')
        plt.savefig('{}/seismic_{}.png'.format(save_path,str(index)))
        plt.imshow(fault[index+286,0:352,0:352])
        plt.axis('off')
